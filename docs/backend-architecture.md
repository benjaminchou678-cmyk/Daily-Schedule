...
| `GET` | `/api/reminders/daily?date=YYYY-MM-DD` | Read pending in-page daily reminders |
| `POST` | `/api/reminders/daily/complete` | Persist a daily reminder as done, skipped, or closed |
| `POST` | `/api/session/heartbeat` | Record an active browser session heartbeat |
| `POST` | `/api/session/closing-intent` | Mark that the browser page is being actively closed |
| `POST` | `/api/notifications` | Send a desktop notification |
| `POST` | `/api/notifications/daily-summary-reminder` | Compatibility endpoint for the close confirmation prompt |
...
- Whether the browser session has likely closed.
- Whether the daily summary close reminder has already been shown today.
...
- Actively closing the website sends a close-intent event. If the session then misses heartbeats, the backend asks whether today's work is finished.
...
The backend calls `listen()` before running startup reminders. Non-core startup work is queued in the background, and failures are logged without stopping the HTTP server. The opener polls `/api/health` for a short bounded window: if the backend is ready it opens `http://127.0.0.1:5173`; otherwise it opens `schedule.html`, a lightweight bootstrap page that polls the health endpoint and redirects when ready.

In-page daily reminders for sport, important schedules, and GitHub maintenance are generated by `backend/services/dailyReminderService.mjs` and persisted by `backend/services/reminderStateService.mjs`. The frontend displays them with a Promise-based queue so only one daily reminder is visible at a time.

Browser close detection is owned by `backend/services/sessionService.mjs`. The page posts `/api/session/heartbeat` every 5 seconds and sends `/api/session/closing-intent` during page shutdown. If a session with close intent misses heartbeats for more than 15 seconds, with an additional refresh grace window, the monitor treats it as an active close. It then shows a Windows yes/no prompt asking whether today's work is finished; choosing yes reminds the user to complete the daily summary.
