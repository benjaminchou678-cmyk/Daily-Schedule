...
Browser close detection is owned by `backend/services/sessionService.mjs`. The page posts `/api/session/heartbeat` every 5 seconds. If a session misses heartbeats for more than 15 seconds, with an additional refresh grace window, the monitor treats it as a possible close. It only shows the Windows daily summary prompt after 20:00 local time, using `backend/services/timeService.mjs` rather than UTC string slicing.
